...
function draw() {
    ...
    background.onload = function(){
        ...
        if(position.value === 'bottom' && wrapTextTest(ctx, headlineInput.value, 10, headlinePostion, 350 * 1.4, parseInt(textPosition) + 4, headlineWrap) === true && svgInput.value.length == 0) {
            headlineWrap = wrapText(ctx, headlineInput.value, 10, headlinePostionWrap, 350, parseInt(textPosition) + 2, headlineWrap);
        }
        else if(svgInput.value.length == 0) {
            headlineWrap = wrapText(ctx, headlineInput.value, 10, headlinePostion, 350, parseInt(textPosition), headlineWrap);
        } else {
        ...
        if(headlineWrap === true && position.value === 'top'){
            wrapText(ctx, standfirstAppend, 10, standyPostion * 2.4, 300, parseInt(standPosition), standfirstWrap);
            ...
            if(svgInput.value.length == 0) {
                wrapText(ctx, standfirstAppend, 10, standyPostion, 300, parseInt(standPosition), standfirstWrap);
            }else {
